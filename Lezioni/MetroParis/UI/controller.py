...
class Controller:
    ...
    def handlePercorso(self, e):
        ...
        if path == []:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(ft.Text("Attenzione, percorso non trovato."))
            return

        self._view.lst_result.controls.clear()
        self._view.lst_result.controls.append(ft.Text(f"Percorso trovato!"))
        self._view.lst_result.controls.append(ft.Text(
            f"Il cammino più breve fra {self._fermataPartenza} e {self._fermataArrivo}, impiega {totTime} minuti"))
        for p in path:
